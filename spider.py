...
from saver import SaveHandler
from utils import CookieHandler, ProxyHandler
...
class LagouSpider(object):
    def __init__(self):
        ...
        self.bf = redisbloomfilter.BloomFilter(db=0, key=LAGOU_DONE, password='qwe123')
        self.extract = lambda elem, index: elem[index] if elem else ''

    async def fetch(self, sem, num, session):
        """下载器"""
        # proxy = eval(self.proxy_handler.take_proxy())['https']
    ...
    async def downloader(self, ids, loop):
        ...
        tasks = []
        async with aiohttp.ClientSession(cookies=self.cookie_handler.get_cookie(), loop=loop) as session:
            for num in ids:
                # if self.redis_conn.sismember(self.lagou_blank, num) or self.redis_conn.sismember(self.lagou_done, num):
                if self.bf.exists(str(num)):
                    pass
                else:
    ...
    def parse(self, html, num):
        ...
        if blank:
            ...
            position = self.extract(re.findall(r'<span class="name">(.*?)</span>', html), 0)
            company = self.extract(re.findall(r'<h2 class="fl">(.*?)<i', html, re.S), 0).strip()
            publish_time = self.extract(re.findall(r'<p class="publish_time">(.*?)&nbsp;', html, re.S), 0)
            info = self.extract(re.findall(r'<dd class="job_request">.*?<p>(.*?)</p>', html, re.S), 0)

            info_list = re.findall(r'>(.*?)</span>', info)
            salary = self.extract(info_list, 0).replace(' ', '')
            city = self.extract(info_list, 1).replace(' ', '').replace('/', '')
            experience = self.extract(info_list, 2).replace(' ', '').replace('/', '')
            edu = self.extract(info_list, 3).replace(' ', '').replace('/', '')
            work_time = self.extract(info_list, 4)
            addr_ = re.findall(r'bizArea=.*?#filterBox">.*?</a>(.*?)<a rel="nofollow" href="javascript:;" id="mapPreview">查看地图</a>',
                               html, re.S)
            addr = self.extract(addr_, 0).replace(' ', '').replace('\n', '').replace('-', '')
    ...
    def check_and_save(self, loop):
        ...
        if check < 1:
            print('代理可用度低, 正在更换代理..')
            # self.proxy_handler.update_proxy()
            self.redis_conn.delete(self.lagou_cookies)
            self.cookie_handler.start_save_cookie()
        if check:
            self.save_handler.start_saver(loop)
